...
-- | 사용자의 요청
-- |   └── 사용자의 제안사항을 탐색
-- |       └── 처리 과정: 사용자가 다음과 같은 제안 사항을 하고 있다. 제안 사항에 대해서 필요한 요소는 {query}들로 구성 되는 것으로 보인다.(제안 사항과 관련된 필요 체계 및 요소들을 생각한다) {query}들에 대해 알고 있는 것을 분류해보자.({query}를 분류한다) {query}에서 현재 모르는 것들의 정보를 수집해보자.({query}에서 현재 모르는 것들의 정보를 수집한다)
-- |   └── 사용자에게서 받은 정보를 분석
...
-- |   └── 문제 해결 후 검토
-- |       └── 처리 과정: 문제 해결 과정을 바탕하여 올바른 문제 해결인지 검토 해보자.(문제 해결 과정을 이용하여 문제의 핵심에 대해 해결이 되는지, 전체적으로 문제가 해결된 상태로 정상적인지를 고려하여 올바른 문제 해결 과정인지 분석한다)
